...
from tpu.tpu_model import get_keypoints, preprocess_frame_for_tpu  # TPU model keypoints detection
...
prev_x_min, prev_y_min, prev_x_max, prev_y_max = None, None, None, None
smooth_factor = 0.6  # Adjust this value to control smoothing
min_box_height = 150  # Minimum height for the box (represents rough estimate of body size)
min_box_width = 100   # Minimum width for the box (represents rough estimate of body size)

def gen_frames():
    global servo_moved_clockwise, servo_moved_counterclockwise, last_movement_time, auto_servo_movement
    global prev_x_min, prev_y_min, prev_x_max, prev_y_max
    ...
    while True:
        ...
        if keypoints is not None and keypoints.any():
            ...
            # Select key keypoints (e.g., head, shoulders, hips)
            important_points = [0, 5, 6, 11, 12]  # Example indices for head, shoulders, hips (adjust as needed)

            # Filter out visible keypoints from the important points
            x_coords = [int(kp[1] * frame_width) for i, kp in enumerate(keypoints) if i in important_points and kp[2] > 0.5]
            y_coords = [int(kp[0] * frame_height) for i, kp in enumerate(keypoints) if i in important_points and kp[2] > 0.5]

            # Use a fallback if we have too few points to estimate body size
            if len(x_coords) < 2 or len(y_coords) < 2:
                if x_coords and y_coords:
                    # If only one keypoint (e.g., head) is available, estimate full body
                    x_min = max(x_coords[0] - min_box_width // 2, 0)
                    y_min = max(y_coords[0] - min_box_height // 2, 0)
                    x_max = min(x_coords[0] + min_box_width // 2, frame_width)
                    y_max = min(y_coords[0] + min_box_height // 2, frame_height)
                else:
                    # If no points are detected, skip drawing the box for this frame
                    continue
            else:
                # Calculate the top-left and bottom-right corners of the bounding box from keypoints
                x_min = min(x_coords) - 50  # Add padding
                y_min = min(y_coords) - 50
                x_max = max(x_coords) + 50
                y_max = max(y_coords) + 50

            # Apply smoothing to avoid jitter
            if prev_x_min is not None:
                x_min = int(smooth_factor * prev_x_min + (1 - smooth_factor) * x_min)
                y_min = int(smooth_factor * prev_y_min + (1 - smooth_factor) * y_min)
                x_max = int(smooth_factor * prev_x_max + (1 - smooth_factor) * x_max)
                y_max = int(smooth_factor * prev_y_max + (1 - smooth_factor) * y_max)

            # Store the current bounding box for smoothing next frame
